...
(function ($, window, document) {
    ...
    function Plugin(el, options) {
        ...
        elem.on('tooltip:show ' + options.trigger.show, function () {
            if ((elem.attr('title') == "" || elem.attr('title') == undefined) && elem.attr('original-title') == undefined) {
                return;
            }
            ...
            tip.stop().delay(options.delay.show).fadeTo(options.fade ? options.fade.duration : 0, options.fade.opacity || 0.7, options.fade.complete);
        });
